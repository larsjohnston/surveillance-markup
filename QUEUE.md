...
Main at `22a6812`. Save schema v34.
...
- **Elevator controls** — Layer A landed (I/O board `B-ACS6100-IO` placeable via Controllers tier-2). Remaining: call station SKU + per-floor access mapping + cab-reader flag (`dev.elevator = {isCabReader, floorsServed}`) + floor-select relay qty derivation (sum of floorsServed across cab readers, rounded up to nearest 8 = board capacity).
...
- **AC Hardware Auto-Spec M1** (PR #60 / `22a6812`) — Project-level `projectInfo.acHardwareConfig` ({mode, powerMode}) drives a new `computeAutoRows` AC emitter that sizes Brivo panel + expansion boards + (optional) external PSU based on placed-reader count (pageMultiplier-aware). Ruleset: D=1 → ACS100 Mullion (suppresses placed-Reader §2.1 rows — panel-is-reader); D=2 → ACS300; D=3-6 → ACS6100R + ceil((D-2)/2) DB; D=7-14 → ACS6100L + DB; D=15-22 → L + R-EXP chained + DB; D=23-28 → L + L-EXP chained + DB; D≥29 → warning row. Power tier (D≥3): internal PSB (free, default, warns at overflow) / LSP SV75 or SV150 / Altronix Trove1/2/3BV. IO board count from placed B-ACS6100-IO devices feeds boardCount for PSU sizing. Project Info modal gains "Access Control Hardware Auto-Spec" section with Mode pill (Off | On) + Power Mode pill (Internal PSB | LSP | Altronix). Default `mode='off'` so legacy/new projects render identically until integrator opts in. Save schema v33→v34 (additive — applyProjectState backfills on older saves). Lock-power scope deferred to M2.
- **Elevator M1 — I/O board** (PR #58 / `4ca644f`) — `B-ACS6100-IO` added to `BRIVO_CATALOG` as a standalone Controllers tier-2 tile (after ACS100, no variants — mirrors ACS300's `variant_axis: null` shape). Drill-down: AC mode → Controllers → ACS6100-IO → click to arm placement. BOM auto-emits one row per placement into §2.1 Access Control via the existing `computeAutoRows` AC emitter; qty multiplies per page-typical like every other AC controller. Pricing resolved from `pricingBook.json` (cost $564.90 / msrp $1,129.80 — "Brivo Access Controller Input/Output Expansion Board — 8 inputs and 8 outputs"). New `AC_ICONS['ACS6100-IO']` SVG (board outline with 8-position terminal strips top + bottom + centre bar). No schema bump, no new BOM section. Layer A only — cab-reader flag + per-floor relay derivation + call-station SKU remain queued under the broader Elevator module.
- **Credentials M2** (PR #56 / `7f41d89`) — Farpointe overhead-door SKUs added to credentials drill-down. `BRIVO_CRED_CATALOG` gains 2 entries (`FP-WRR22` Long-Range Receiver + `FP-WRT2B` 2-Button Transmitter) under new `tier2_family: 'Overhead Doors'`, each carrying optional `section: '2.2'`. brivoSkus BOM emitter changes hardcoded `section: '2.1'` → `cat.section || '2.1'` so OH-family rows land in §2.2 Overhead Door Control while all existing 11 catalog entries continue routing to §2.1 unchanged. `_renderCredentialsTier3` `FAMILY_ORDER` extended with 'Overhead Doors'. Legacy `CREDENTIAL_TYPE_DEFS` ohTransmitters + ohReceiver placeholder entries dropped (no UI wrote to them since the 5-tile modal was retired). `applyProjectState` migrates any saved `projectInfo.credentials.ohTransmitters` / `ohReceiver` qtys additively into `brivoSkus['FP-WRT2B']` / `brivoSkus['FP-WRR22']` and zeroes the legacy fields (idempotent on re-load). Save v32→v33. `pricingBook.json` untouched (FP-* SKUs already present with cost+msrp). Farpointe is the actual manufacturer; SKUs ship via the Brivo Excel sheet.
- **DoorBird M1** (PR #55 / `e99fbde`) — Intercom placement auto-opens right pane (diverges from sticky-placement convention used by camera/reader/parcel/mailbox). DoorBird-only right-pane mounting field with 2 checkboxes: Surface-mount (adds device-specific backbox row, per-INTERCOM_DB-entry `bracketSku`; D31TDV → `DB-D31TDV-BB-SSV2A` wired) + Pedestal (adds universal 45" gooseneck pedestal `33PE1-DOOR-01-CRS`, MSRP $1400). New `dev.surfaceMount` + `dev.pedestal` booleans on intercom dev objects; BOM emitter sums into §2.3 accessory auto-rows. `bomRowSku` gains `src === 'smartApartment' && row.sku` precedence so prefixed accessory keys resolve to the right pricing-book SKU. Save schema v31→v32 (defensive backfill). Pedestal pricing seeded in `source-data/doorbird-extract-template.csv`.
- **IoT M1** (PR #54 / `259e0e3`) — §4.2 IoT manual pricing (mirror §2.4 + editable Description). `_sqResolveAutoRowSell` gate widened from `'2.4'` literal to `MANUAL_PRICED = { '2.4': 1, '4.2': 1 }`. §4.2 auto-rows get editable Cost+Sell+Description inputs (user types "Yale Assure SL" per project); §2.4 keeps fixed descs. Description override rides `bomAutoOverrides[key].desc` (compute-wrapper writes through to `r.desc`). `updateAutoRow` now field-dispatches (text vs money). CSS: `.sechw-unpriced-flag` → `.bom-unpriced-flag` (semantic rename); new `.bom-desc-edit-cell` flex wrapper. Pricing-book template seeded at `source-data/iot-extract-template.csv` (no adapter yet — future pass).
...
- **SUB-M2** (PR #43 / `28fcbb4`) — Eagle Eye per-camera VMS subscription. New 6.2 Camera VMS sub-section with retention pill, project-wide `cameraStorageRetention`, EN-PR1 desc strip-from-paren. Save v30.
