...
_Canonical manually-maintained queue. Last regenerated at main `678e69f`, after the banded-BOM-header styling pass._

---

## THE REFRAME (terminology — affects everything below)

- **Security Quote** = the on-screen "BOM" drawer/modal (renamed). Internal **priced** workspace; sibling to the Door Hardware wizard. Floor-plan placements → materials → SKU → (eventually) price-book lookup → margin/tax/totals. Pricing machinery STAYS here.
- **Door Hardware Quote** = the existing DHW wizard (import → price → award → markup → schedule).
- **Bill of Materials** = the **customer-facing proposal-export PDF page only**. Qty · SKU · Description, **no pricing**. Two-line rows, navy/blue banded section headers.
...
- **BOM two-tier restructure** — `43fdd4a`, tag `bom-two-tier-pass`. M1–M8 (M5 N/A). Flat 6-section → 5 majors + "Other"; one `computeBomTree()` feeds drawer + PDF + CSV. Placed-switch row (M1), IoT expansion gateway/keypad/passageSet + **save v18→v19** (M2), two-tier on-screen + PDF (M3/M4), DHW section-5 classifier (M6), 2.2 dedup guard + Unclassified amber (M7), CSV tier columns + latent `&amp;` fix (M8).
- **Centered BOM modal** — `45d1afd` (CSS-only: right drawer → centered 2/3 modal).
- **Customer BOM export page** — `29d0ccd`. Rename drawer/menu → "Security Quote"; PDF BOM page → two-line Qty·SKU·Description, no pricing, larger font, SKU-vs-typeLabel via color+brackets; `sku` attached to `dhwBomRows()`.
- **Banded BOM headers** — `678e69f`. Customer BOM PDF section headers → filled navy tier-1 bands + blue tier-2 bands, white bold text, "QTY" label, matching the SMART MF proposal aesthetic (ref: Alpine proposal p.3). Hardcoded RGB navy `(40,54,92)` / blue `(74,110,180)` with a TODO to unify with `branding.palette`.

---

## 🔴 HARD DEPENDENCY — resolve before any client-facing export

- **Cover GRAND TOTAL shows $0.00.** The customer BOM page is price-free, and `drawProposalCover` sums `computeBomTree()` × margin/tax = $0 (all unit prices 0 until pricing lands). A proposal exported today shows a **$0 cover — looks broken to a client.** Resolved by EITHER the cover-redesign branch (hide/omit the $0 total) OR Pass 2 (real prices). Do not ship a client proposal until one lands.
...
- **Promote `sku` to a first-class material field** — the BOM page derives SKU at render; pricing needs a real per-material SKU to key the lookup. Cameras/AC/intercom/parcel/DHW have a source; mailbox/IoT/derived need SKU assignment or a no-price marker.
...
- **Proposal cover redesign** — SMART-MF-branded title page (angled navy/white jsPDF geometry; logo + photo base64; Project/Client/Date). Renders from `projectInfo.branding` (multi-tenant-ready, SMART MF default). Scope + summary-counts relocate to their own page. Helvetica not courier.
  ...
  - No version bump (branding backfill-on-load). Rebase onto current main before merge.
  - Could also resolve the $0-cover dependency (hide $0 total).
  - **ON MERGE — palette unification (REQUIRED):** the banded BOM headers hardcode navy `(40,54,92)` / blue `(74,110,180)` with a TODO. When this branch lands `branding.palette`, reconcile BOTH into one palette object so the proposal doesn't run two slightly-different navies across pages (cover vs BOM page). Hits the "consistent on every module" rule. Also reconcile against the older gray-navy `(31,41,55)` used on the cover header / page headers / take-off — decide one canonical brand navy for the whole proposal.
...
- **Unify proposal navy/blue into one palette** — see the cover-branch merge note above. Currently three values float: BOM-band navy `(40,54,92)` + blue `(74,110,180)`, and the older gray-navy `(31,41,55)` on cover header / page headers / take-off. Pick one canonical brand navy when `branding.palette` lands.
- **"QTY" band label is decorative** on the BOM page — rows carry Qty inline (`· Qty N` on line 1), so nothing sits under the label. Either drop it or right-align line-1 Qty under it (the page-3-faithful option). Small follow-up.
- **Track pass briefs in the closing commit.** Recurring: `git add` lines named only `camera_markup_tool.html`, so briefs dangled untracked. Fold the brief filename into the pass's final commit.
- **Commit-per-milestone discipline.** The BOM two-tier pass rode 8 uncommitted milestones on the OneDrive tree all session (recovered fine, but fragile). Commit after each browser review.
- **Resync anchor on every CC block.** Driving git in raw terminal + via CC causes stale-state drift (multiple collisions this session). Each CC block opens with `[HEAD: <hash> | branch | tree]`.
...
- **Defer `bom*` code-symbol rename** — UI says "Security Quote", symbols stay `bom*`; high-churn/zero-gain, do only if it ever causes confusion.
...
- **User guide** — wizard + W7/W8 + Hardware Module + cleanup stretch + BOM two-tier + centered modal + Security-Quote rename + customer BOM page + banded headers all undocumented.
...
- ~~Navy/blue banded BOM section headers~~ — `678e69f`.
- ~~Placed switch in BOM / IoT expansion / DHW section-5 classifier~~ — BOM two-tier M1/M2/M6.
