...
### §2.4 Security Hardware — manual-pricing exception (auto-row Cost+Sell editable)

§2.4 is the ONLY section where an auto row's Sell is hand-typed rather than rule-derived. The pricing rule does NOT apply to §2.4. Implementation: a single `_sqResolveAutoRowSell(row, sectionId, rule)` helper gates on `sectionId==='2.4'` (reads `bomAutoOverrides[_normalizeBomKey(row.key)].sell`, fallback 0) and is called from ALL FIVE auto-row sell sites — recalcBom totals, `_sqRenderAutoRow`, Summary totals, `_sqRenderSummaryXlsxAutoRow`, `_wpRenderQuote`, `drawProposalQuote`. Adding a 6th sell site WITHOUT routing it through this helper reintroduces the Materials-vs-output divergence bug (same class as the credentials $0 bug). Cost stays the standard editable `data-field="unit"` input; Sell is a `data-field="sell"` input wrapped in `.bom-cell-sell-edit` (NOT `.bom-cell-sell` — that lacks the input sizing and overflows into Qty). Both write to `bomAutoOverrides[key]` via field-agnostic `updateAutoRow`. The "unpriced" badge is synced in TWO places: `_sqRenderAutoRow` (full render) and `_sqRecalcRowCells` (in-row edit) — both must stay in sync or the badge goes stale on edit. KNOWN HARMLESS: `_sqRecalcRowCells`'s `.bom-cell-sell` textContent write is a no-op on §2.4 (the cell is `.bom-cell-sell-edit`); the input already shows the value. Qty stays locked/auto on §2.4 like every other auto row.
