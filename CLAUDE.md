...
- **Camera database** — 25 Eagle Eye + 12 Hanwha Wisenet models with real specs (8 EOL Hanwha removed); each Hanwha entry carries `sku:` keyed to the active HVA pricelist
...
- **Per-reader Security Hardware (§2.4)** — each placed AC Reader auto-attaches a security-hardware bundle (Electric Strike / REX / Door Position Sensor), toggled via 3 checkboxes in the reader right-panel (Reader-only; Controllers excluded). Bundle stored on `dev.secHw = {strike,rex,dps}` (undefined until first panel-open seeds all three on). Rolls up into SQ MATERIALS peer subsection "2.4 Security Hardware" (group-by-component, qty = count of readers with that component on), and into web proposal + retained PDF. §2.4 is MANUALLY PRICED — Cost AND Sell are editable inputs (default $0, no pricing rule applied, Sell typed not derived, stored in `bomAutoOverrides[key].sell`); an amber "unpriced" badge flags any row with Cost or Sell = $0. Generic book-less SKUs (Estrike / REX / DPS). Predated-file load fires a one-time migration prompt ("Add security hardware?") — Yes seeds all readers, No leaves empty; both set `projectInfo.secHwMigrationDismissed=true` (never re-prompt; dev_quiet-suppressed).

## Pending / known gaps
- **Pricing is cloud-loaded** from `pricingBook.json` (2655 items, 6 vendors: Eagle Eye, Brivo Access, Brivo Credentials, Hanwha Vision, DoorBird, Luxer One) via `fetchRemotePricing()` on boot; localStorage cache fallback when offline. Regenerated by `build_pricing_json.py` (untracked, repo root; per-vendor adapters). Hanwha = `parse_hanwha` xlsx-direct adapter ("HVA Pricelist" sheet, cost = 42%-off column = MSRP × 0.58). Catalog SKUs must match book keys exactly (no normalization).
...
- v34 — AC Hardware Auto-Spec M1 (`projectInfo.acHardwareConfig` with `mode` ('off'|'on') and `powerMode` ('internal'|'lsp'|'altronix')). When mode='on', `computeAutoRows` emits Brivo panel + expansion-board + (optional) external-PSU rows sized to door count = reader count (pageMultiplier-aware): D=1 → ACS100 Mullion (suppresses placed-Reader §2.1 rows since panel-is-reader); D=2 → ACS300; D=3-6 → ACS6100R + DB; D=7-14 → ACS6100L + DB; D=15-22 → L + chained R-EXP + DB; D=23-28 → L + chained L-EXP + DB; D≥29 → warning row (single-MBE 14-board limit). Power tier (D≥3 only; small panels assume PoE): internal PSB (free, default) + warn if boardCount > capacity; LSP → SV75/SV150 based on boardCount; Altronix → Trove1/2/3BV based on boardCount. IO board count (placed B-ACS6100-IO from Elevator M1) feeds boardCount for PSU sizing. Default mode='off' so existing/new projects render IDENTICALLY to pre-feature behavior until the integrator opts in via Project Info modal → "Access Control Hardware Auto-Spec" section. Lock-power scope deferred to M2.
- v28 — DHW B-lite
...
### Manually-priced sections — §2.4 Security Hardware + §4.2 IoT (auto-row Cost+Sell editable)

§2.4 and §4.2 are the TWO sections where an auto row's Sell is hand-typed rather than rule-derived. The pricing rule does NOT apply to either. Implementation: a single `_sqResolveAutoRowSell(row, sectionId, rule)` helper carries a `MANUAL_PRICED = { '2.4': 1, '4.2': 1 }` set; if the row's section matches, it reads `bomAutoOverrides[_normalizeBomKey(row.key)].sell` (fallback 0); non-matching sections fall through to `deriveSell`. The helper is called from ALL FIVE auto-row sell sites — recalcBom totals, `_sqRenderAutoRow`, Summary totals, `_sqRenderSummaryXlsxAutoRow`, `_wpRenderQuote`, `drawProposalQuote`. Adding a 6th sell site WITHOUT routing it through this helper reintroduces the Materials-vs-output divergence bug (same class as the credentials $0 bug). Adding a new manually-priced section is a single MANUAL_PRICED key add — propagates everywhere.

Cost stays the standard editable `data-field="unit"` input; Sell is a `data-field="sell"` input wrapped in `.bom-cell-sell-edit` (NOT `.bom-cell-sell` — that lacks the input sizing and overflows into Qty). Both write to `bomAutoOverrides[key]` via field-agnostic `updateAutoRow`. The "unpriced" badge (`.bom-unpriced-flag`) is synced in TWO places: `_sqRenderAutoRow` (full render) and `_sqRecalcRowCells` (in-row edit) — both must stay in sync or the badge goes stale on edit.
